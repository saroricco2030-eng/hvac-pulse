...
const Auth = (() => {
  ...
  function renderLogin(container) {
    container.innerHTML = `
      <div class="glass-card auth-card">
        ...
        <p class="auth-desc">
          로그인하면 수리기록과 메모를 클라우드에 백업할 수 있습니다.
        </p>
        ...
        <p class="auth-note">
          ...
          로그인은 선택사항입니다. 오프라인에서도 모든 기능을 사용할 수 있습니다.
        </p>
      </div>`;
  }
      ...
      <div class="glass-card auth-card">
        ...
        <div class="auth-profile">
          <div class="auth-avatar">
            ${photoURL
              ? `<img src="${photoURL}" alt="프로필" referrerpolicy="no-referrer">`
  ...
  function showEmailModal(mode) {
    ...
    overlay.innerHTML = `
      <div class="auth-modal">
        ...
        <div class="auth-modal-title">${mode === 'login' ? '이메일 로그인' : '회원가입'}</div>
        ...
        <div class="form-group">
          ...
          <input type="password" id="auth-password" class="form-input" placeholder="6자 이상"
        ...
        <div class="auth-modal-footer">
          ${mode === 'login'
            ? `<span>계정이 없으신가요? <a href="#" onclick="Auth.showEmailModal('signup');return false">회원가입</a></span>
               <span><a href="#" onclick="Auth.resetPassword();return false">비밀번호 재설정</a></span>`
            : `<span>이미 계정이 있으신가요? <a href="#" onclick="Auth.showEmailModal('login');return false">로그인</a></span>`
          }
        </div>
      </div>`;
  ...
  async function submitEmail() {
    ...
    try {
      if (mode === 'signup') {
        ...
        App.showToast('회원가입 완료! 클라우드 동기화가 활성화됩니다.', 'success');
      } else {
        await firebase.auth().signInWithEmailAndPassword(email, password);
        App.showToast('로그인 성공!', 'success');
      }
      closeEmailModal();
    } catch (err) {
      const errorMessages = {
      ...
      showAuthError(errorMessages[err.code] || `오류: ${err.message}`);
      if (submitBtn) {
  ...
  async function loginWithGoogle() {
    ...
    try {
      ...
      App.showToast('Google 로그인 성공!', 'success');
    } catch (err) {
      if (err.code === 'auth/popup-blocked') {
        App.showToast('팝업이 차단되었습니다. 팝업을 허용해주세요.', 'warning');
      } else if (err.code === 'auth/popup-closed-by-user') {
        // User closed popup — no action needed
      } else {
        App.showToast('Google 로그인 실패: ' + err.message, 'error');
      }
    }
  }
  ...
  async function resetPassword() {
    ...
    try {
      ...
      showAuthError('');
      App.showToast('비밀번호 재설정 이메일을 발송했습니다.', 'success');
    } catch (err) {
      showAuthError('이메일 발송 실패. 이메일 주소를 확인하세요.');
    }
  }
