          ...
          <div style="display:flex;gap:4px;flex-wrap:wrap">
            ${recentSearches.slice(0, 5).map(q => `
              <button onclick="ErrorCodeSearch.quickSearch('${q.replace(/'/g, "\\'")}')"
                style="padding:3px 8px;font-size:var(--text-xs);background:var(--bg-card);border:1px solid var(--border);border-radius:12px;color:var(--text-muted);cursor:pointer;font-family:var(--font-sans)">${q}</button>
            `).join('')}
          </div>
        ` : ''}
      </div>
          ...
          ${ERROR_CODE_MANUFACTURERS.map(m => {
            ...
            return `
              <button onclick="ErrorCodeSearch.showManufacturerCodes('${m.id}')"
                style="display:flex;align-items:center;gap:8px;padding:12px;background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-md);color:var(--text-primary);cursor:pointer;font-family:var(--font-sans);font-size:var(--text-sm);text-align:left;min-height:48px;transition:all 0.2s ease"
    ...
    if (matches.length === 0) {
      resultEl.innerHTML = `
        <div class="glass-card" style="text-align:center;padding:32px 24px">
          ...
          <p style="color:var(--text-muted);font-size:var(--text-sm);margin-top:4px">${t('errorcode.try_other', '다른 키워드로 검색해 보세요.')}</p>
        </div>`;
      return;
    }
    ...
    return `
      <div class="glass-card ec-card" style="padding:14px;margin-bottom:8px;border-left:3px solid ${sev.color}"
        ...
        <button onclick="ErrorCodeSearch.toggleExpand('${cardId}')"
          style="margin-top:8px;padding:6px 12px;font-size:var(--text-sm);background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text-secondary);cursor:pointer;font-family:var(--font-sans);width:100%;text-align:center">
        ...
        <div id="ec-detail-${cardId}" style="display:${isExpanded ? 'block' : 'none'};margin-top:12px">
          ...
          <div style="margin-bottom:10px">
            <div style="font-size:var(--text-xs);font-weight:600;color:var(--accent-red);margin-bottom:4px">${t('errorcode.causes', '원인')}</div>
            <ul style="margin:0 0 0 16px;font-size:var(--text-sm);color:var(--text-secondary);line-height:1.7">
          ...
          <!-- Check steps -->
          <div style="margin-bottom:10px">
            <div style="font-size:var(--text-xs);font-weight:600;color:var(--accent-green);margin-bottom:4px">${t('errorcode.check_steps', '점검 순서')}</div>
            <ol style="margin:0 0 0 16px;font-size:var(--text-sm);color:var(--text-secondary);line-height:1.7">
  ...
  function _renderRelatedDiagnostics(errorCode) {
    ...
    diagMatches.forEach(dk => {
      html += `<button class="ec-diag-link-btn" onclick="App.switchTab('tools');setTimeout(()=>{App.showCategory('diag');setTimeout(()=>App.showSub('tools','cross'),50)},50)">
        ...
        <span style="margin-left:auto;color:var(--text-muted);font-size:var(--text-xs)">${t('errorcode.cross_diag', '교차진단')} →</span>
      </button>`;
    });
